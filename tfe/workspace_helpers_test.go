...
func TestReadWorkspaceStateConsumers(t *testing.T) {
	cases := []struct {
		...
		expectedRemoteStateConsumerIds []interface{}
	}{
		{
			remoteStateConsumersResponse:   "200",
			err:                            false,
			expectedGlobalRemoteState:      false,
			expectedRemoteStateConsumerIds: make([]interface{}, 0),
		},
		{
	...
	for _, tc := range cases {
		...
		actualGlobalRemoteState, actualRemoteStateConsumerIds, err := readWorkspaceStateConsumers(&tfe.Workspace{ID: "ws-123"}, client)
		if (err != nil) != tc.err {
			t.Fatalf("expected error is %t, got %v", tc.err, err)
		}
